window.onload = function () {
    ...
    // 数据源
    let polyLineArr = [116.397428, 39.90923, 117, 39, 116.39, 39.8];
    ...
    polyLineArr.forEach((item, index) => {
        ...
        if (index % 2 === 0) {
            startIndex = index;
            lastIndex = index + 2;
            let p = polyLineArr.slice(startIndex, lastIndex);
            if (p) {
                let position = Cesium.Cartesian3.fromDegrees(p[0], p[1], 1); // 高度为1
                // 开始时间
                if (
                    startIndex === 0
                ) {
                    startTime = Cesium.JulianDate.now();
                    SampledPositionProperty.addSample(startTime, position);
                } else {
                    endTime = Cesium.JulianDate.addSeconds(startTime, time *= 2, new Cesium.JulianDate());
                    SampledPositionProperty.addSample(endTime, position);
                }
            }
        }
    })
